...
import acosta_bonafede_spadola_unzaga.DAO.AsistidoDAO;
...
public class AsistidoServiceImp implements AsistidoService {
    ...
    public boolean existsByNombreCompletoButNotId(String nombreCompleto, Long id) {
        // Desglosar el nombre (asumiendo que el DAO tiene el método)
        String[] partes = nombreCompleto.split(" ", 2);
        String nombre = partes[0];
        String apellido = partes.length > 1 ? partes[1] : "";
        
        // Se asume la existencia de findByNombreAndApellidoAndIdNot en el DAO.
        return !repoAsistido.findByNombreAndApellidoAndIdNot(nombre, apellido, id).isEmpty();
    }
    ...
    public boolean existsByNombreCompleto(String nombreCompleto) {
        // Se asume que el nombre completo en el DTO es "Nombre Apellido"
        String[] partes = nombreCompleto.split(" ", 2);
        String nombre = partes[0];
        String apellido = partes.length > 1 ? partes[1] : "";
        
        // CORRECCIÓN: Usamos isEmpty() en la lista y negamos el resultado.
        return !repoAsistido.findByNombreAndApellido(nombre, apellido).isEmpty();
    }
