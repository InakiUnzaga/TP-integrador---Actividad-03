...
import acosta_bonafede_spadola_unzaga.Presentation.ciudades.CiudadRestController;
//import acosta_bonafede_spadola_unzaga.Presentation.asistencias.AsistenciaRestController;  //Asumido para link S03
import acosta_bonafede_spadola_unzaga.Service.AsistidoService;
import acosta_bonafede_spadola_unzaga.Service.CiudadService;
import acosta_bonafede_spadola_unzaga.error.MensajeError;
...
public class AsistidoRestController {
	...
	public ResponseEntity<Object> insertar( @Valid @RequestBody AsistidoDTO dto, BindingResult result) throws Exception
	{
		...
		if(service.existsByNombreCompleto(dto.getNombreCompleto())) {
	...
	public ResponseEntity<Object> actualizar(@Valid @RequestBody AsistidoDTO dto, @PathVariable long id, BindingResult result) throws Exception
	{
       ...
       //Nombre Completo
        String nombreCompletoOriginal = asistidoOriginal.getNombre() + " " + asistidoOriginal.getApellido();
        String nombreCompletoNuevo = dto.getNombreCompleto(); // Nombre completo del DTO
     
        if (!nombreCompletoOriginal.equalsIgnoreCase(nombreCompletoNuevo)) {          
            if (service.existsByNombreCompletoButNotId(nombreCompletoNuevo, id)) {
